...
function SharedCommonFiles()
	...
	if not shared_common_files then
		...
		local game_content_source = ContentCompile("game_content_source", "generated/game_data.cpp")
		local game_content_header = ContentCompile("game_content_header", "generated/game_data.h")
...
function SharedIcons(compiler)
	if not shared_icons[compiler] then
		local server_icon = ResCompile("other/icons/ninslash_srv_" .. compiler .. ".rc", compiler)
		local client_icon = ResCompile("other/icons/ninslash_" .. compiler .. ".rc", compiler)
...
function SharedManifests(compiler)
	if not shared_manifests then
		local client_manifest = ResCompile("other/manifest/ninslash.rc", compiler)
...
function BuildClient(settings, family, platform)
	...
	local game_client = Compile(settings, CollectRecursive("src/game/client/*.cpp"))
	local game_editor = Compile(settings, Collect("src/game/editor/*.cpp"))
	
	Link(settings, "ninslash", libs["zlib"], libs["md5"], libs["wavpack"], libs["png"], libs["json"], client, game_client, game_editor)
end

function BuildServer(settings, family, platform)
	local server = Compile(settings, Collect("src/engine/server/*.cpp"))
	
	local game_server = Compile(settings, CollectRecursive("src/game/server/*.cpp"))
	
	return Link(settings, "ninslash_srv", libs["zlib"], libs["md5"], libs["json"], server, game_server)
end
...
function BuildContent(settings, arch, conf)
	local content = {}
	-- table.insert(content, CopyToDir(settings.link.Output(settings, "data"), CollectRecursive(content_src_dir .. "*.png", content_src_dir .. "*.wv", content_src_dir .. "*.ttc", content_src_dir .. "*.ttf", content_src_dir .. "*.txt", content_src_dir .. "*.map", content_src_dir .. "*.rules", content_src_dir .. "*.json", content_src_dir .. "*.atlas", content_src_dir .. "*.vot", content_src_dir .. "*.npc", content_src_dir .. "*.frag")))
	-- table.insert(content, CopyToDir(settings.link.Output(settings, ""), CollectRecursive(content_src_dir .. cfg_dir .. "*.cfg")))
	-- table.insert(content, CopyFile(settings.link.Output(settings, "") .. "/autoexec.cfg", "autoexec.cfg"))
	-- table.insert(content, CopyFile(settings.link.Output(settings, "") .. "/storage.cfg", "storage.cfg"))
	-- table.insert(content, CopyFile(settings.link.Output(settings, "") .. "/license.txt", "license.txt"))
	if family == "windows" then
		if arch == "x86_64" then
			_arch = "64"
		else
		...
		AddJob({
				...
				"other/glew/include/GL/glew.h", "other/glew/windows/lib" .. _arch .. "/glew32.dll"
		...
		table.insert(content, CopyFile(settings.link.Output(settings, "") .. "/glew32.dll", "other/glew/windows/lib" .. _arch .. "/glew32.dll"))
		AddDependency(settings.link.Output(settings, "") .. "/SDL2.dll", "other/sdl2/include/SDL.h")
		AddDependency(settings.link.Output(settings, "") .. "/freetype.dll", "other/freetype/include/ft2build.h")
		AddDependency(settings.link.Output(settings, "") .. "/glew32.dll", "other/glew/include/GL/glew.h")
	end
...
targets = {client="ninslash", server="ninslash_srv",
